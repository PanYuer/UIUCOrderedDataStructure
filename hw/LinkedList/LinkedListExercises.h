...
void LinkedList<T>::insertOrdered(const T &newData)
{
  ...
  // Node sentinel = Node(newData);
  // sentinel.next = head_;
  // Node *prev = &sentinel, *cur = head_;
  // while (cur && cur->data < newData)
  // {
  //   prev = cur;
  //   cur = cur->next;
  // }
  ...
  // Allocate a new node and store newData there.
  ...
  // Position thru at the head of list.
  //thru = head_;
  ...
  // Skip all nodes whose data is less than newData.
  Node *thru = head_; // To traverse the list
  while (thru->next != nullptr && thru->next->data <= newData)
  {
    thru = thru->next;
  }
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T> &other) const
{
  ...
  // -----------------------------------------------------------
  // TODO: Your code here!
  Node *left_head = left.getHeadPtr();
  Node *right_head = right.getHeadPtr();
  while (left_head && right_head)
  {
    if (left_head->data < right_head->data)
    {
      merged.pushBack(left_head->data);
      left_head = left_head->next;
    }
    else
    {
      merged.pushBack(right_head->data);
