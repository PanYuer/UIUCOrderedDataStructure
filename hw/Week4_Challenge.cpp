...
Again, as is implied by the above description, for this exercise you may assume that only the root node violates the min heap property at the beginning, if any node does (as the left and right subtrees are already valid heaps). This means it's possible to implement the downHeap function as O(log)O(logn). If your algorithm has a running time worse than O(logn)O(logn), it is probably incorrect. The significance of this 
O(logn)O(logn) algorithm is that it can be used as an efficient tool in the O(n)O(n)-time algorithm that corrects a new heap in multiple steps from the bottom up, as described in lecture.
*/
