...
static void treeFactory(GenericTree<int> &tree)
{
  ...
  TreeNode *node_1_0 = root->addChild(8);
  TreeNode *node_1_1 = root->addChild(15);
  TreeNode *node_2_0 = node_1_0->addChild(16);
  TreeNode *node_2_1 = node_1_0->addChild(23);
  TreeNode *node_3_0 = node_2_0->addChild(42);
}
...
std::vector<T> traverseLevels(GenericTree<T> &tree)
{
  ...
  while (!q.empty())
  {
    ...
    for (auto child : node->childrenPtrs)
    {
      if (child)
        q.push(child);
    }
